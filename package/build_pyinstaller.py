...
def create_shell_file(os_type):
    ...
    if os_type == "linux":
        ...
        # Modify by darrin - 20190602 - start
        shell_text = "@echo off\n" + \
                     "del %s" % os.path.join(exec_folder_name, "appdata\\Cache\\*.* /Q\n") + \
                     "del %s" % os.path.join(exec_folder_name, "appdata\\GPUCache\\*.* /Q\n") + \
                     "start %s" % os.path.join(exec_folder_name, "ardublockly.exe")
        shell_location = os.path.join(project_root_dir, "run.bat")
        shell_text_dev = "@echo off\n" + \
                         "del %s" % os.path.join(exec_folder_name, "appdata\\Cache\\*.* /Q\n") + \
                         "del %s" % os.path.join(exec_folder_name, "appdata\\GPUCache\\*.* /Q\n") + \
                         "start %s" % os.path.join(exec_folder_name, "ardublockly.exe --env=development")
        shell_location_dev = os.path.join(project_root_dir, "run_dev.bat")
        # Modify by darrin - 20190602 - end
    else:
    ...
    print(script_tab + "Creating dev shell file into %s" % shell_location_dev)
    try:
        with open(shell_location_dev, "w") as bash_file:
            bash_file.write(shell_text_dev)
    except Exception as e:
        print(script_tab + "%s" % e)
        print(script_tab + "ERROR: Dev shell file to launch the Ardublockly "
                           "application could not be created.")
        return
